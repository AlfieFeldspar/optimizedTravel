...
router.get("/patients/:nurse", (request, response, next) => {
  // Query the pool
  pool.query(
    "SELECT ptTable.ptFirstName, ptTable.ptLastName, ptTable.ptHomeLng, ptTable.ptHomeLat, ptTable.nursingNeed, ptTable.visitPriority FROM ptTable INNER JOIN rnTable ON ptTable.rn_Id=rnTable.rn_Id WHERE rnTable.rn_Id = ?",
